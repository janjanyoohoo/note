...
![image-20210913170528744](https://jianjiandawang.oss-cn-shanghai.aliyuncs.com/Typora/20210913170528.png)
...
流程引擎的配置类（ProcessEngineConfiguration），通过ProcessEngineConfiguration可以创建工作流引擎ProceccEngine，常用的两种方法
...
![image-20210913170608199](https://jianjiandawang.oss-cn-shanghai.aliyuncs.com/Typora/20210913170608.png)

### 活动 Activity

活动是工作或任务的一个通用术语。一个活动可以是一个任务，还可以是一个当前流程的子处理流程； 其次，你还可以为活动指定不同的类型。常见活动如下：

![image-20210913170636389](https://jianjiandawang.oss-cn-shanghai.aliyuncs.com/Typora/20210913170636.png)

### 网关 GateWay

网关用来处理决策，有几种常用网关需要了解：

![image-20210913170648649](https://jianjiandawang.oss-cn-shanghai.aliyuncs.com/Typora/20210913170648.png)
...
![image-20210913170721700](C:/Users/jianjian/AppData/Roaming/Typora/typora-user-images/image-20210913170721700.png)
...
![image-20210913225826687](https://jianjiandawang.oss-cn-shanghai.aliyuncs.com/Typora/20210913225826.png)
...
![image-20210913225815960](https://jianjiandawang.oss-cn-shanghai.aliyuncs.com/Typora/20210913225816.png)
...
![image-20210913225803923](https://jianjiandawang.oss-cn-shanghai.aliyuncs.com/Typora/20210913225803.png)
...
![image-20210920221355452](https://jianjiandawang.oss-cn-shanghai.aliyuncs.com/Typora/20210920221404.png)
...
2)       如果该流程定义下**没有正在运行的流程，则可以用普通删除。**

如果该流程定义下**存在已经运行的流程，使用普通删除报错**，可用**级联删除方法将流程及相关记录全部删除。**
...
1)       deploymentId为**流程部署ID**

2)       resource_name为act_ge_bytearray表中NAME列的值
...
![image-20210920221032335](https://jianjiandawang.oss-cn-shanghai.aliyuncs.com/Typora/20210920221039.png)
...
![image-20210920221437659](https://jianjiandawang.oss-cn-shanghai.aliyuncs.com/Typora/20210920221448.png)
...
![image-20210920221616940](https://jianjiandawang.oss-cn-shanghai.aliyuncs.com/Typora/20210920221617.png)

说明：启动流程实例，流程当前执行到第一个任务结点，此表会插入一条记录表示当前任务的执行情况，如果任务完成则记录删除。

3. SELECT * FROM act_ru_identitylink #任务参与者，记录当前参与任务的用户或组

![image-20210920221639764](https://jianjiandawang.oss-cn-shanghai.aliyuncs.com/Typora/20210920221639.png)

4. SELECT * FROM act_hi_procinst #流程实例历史表

![image-20210920221731491](https://jianjiandawang.oss-cn-shanghai.aliyuncs.com/Typora/20210920221731.png)

流程实例启动，会在此表插入一条记录，流程实例运行完成记录也不会删除。

5.  SELECT * FROM act_hi_taskinst  #任务历史表，记录所有任务

![image-20210920221809127](https://jianjiandawang.oss-cn-shanghai.aliyuncs.com/Typora/20210920221809.png)

开始一个任务，不仅在act_ru_task表插入记录，也会在历史任务表插入一条记录，任务历史表的主键就是任务id，任务完成此表记录不删除。

6.  SELECT * FROM act_hi_actinst  #活动历史表，记录所有活动

![image-20210920221931542](https://jianjiandawang.oss-cn-shanghai.aliyuncs.com/Typora/20210920221931.png)

活动包括任务，所以此表中不仅记录了任务，还记录了流程执行过程的其它活动，比如开始事件、结束事件。相当于是流程日志记录
...
![image-20210920222637912](https://jianjiandawang.oss-cn-shanghai.aliyuncs.com/Typora/20210920222638.png)
...
格式: `${key}` 或 `${obj.key}` 以及 `${obj.getName(1)}`具体参考UEL表达式文档

#### UEL.Value

![image-20210920223437404](https://jianjiandawang.oss-cn-shanghai.aliyuncs.com/Typora/20210920223437.png)	

![image-20210920223423688](https://jianjiandawang.oss-cn-shanghai.aliyuncs.com/Typora/20210920223423.png)	

#### UEL.Method

![image-20210920223415851](https://jianjiandawang.oss-cn-shanghai.aliyuncs.com/Typora/20210920223524.png)	
...
表达式支持解析`基础类型`、 `bean`、` list`、 `array `和` map`，也可作为条件判断。
...
![image-20210920223720855](https://jianjiandawang.oss-cn-shanghai.aliyuncs.com/Typora/20210920223720.png)	
...
![image-20210920223802216](https://jianjiandawang.oss-cn-shanghai.aliyuncs.com/Typora/20210920223802.png)
...
<img src="https://jianjiandawang.oss-cn-shanghai.aliyuncs.com/Typora/20210920223900.png" alt="image-20210920223900082" style="zoom: 50%;" />	
...
<img src="https://jianjiandawang.oss-cn-shanghai.aliyuncs.com/Typora/20210920224014.png" alt="image-20210920224014243" style="zoom:50%;" />	
...
- date
- binary 
- serializable  必须实现serializable  接口且指定serialVersionUID

<img src="https://jianjiandawang.oss-cn-shanghai.aliyuncs.com/Typora/20210920224355.png" alt="image-20210920224355164" style="zoom:50%;" />	

### 作用域

流程变量的作用域可以是`一个流程实例(processInstance)`，或`一个任务(task)`，或一个`执行实例(execution)`
...
- `key：value`结构
  - 举例: userId(变量名),zhangsan(变量值)

- global 变量中变量名**不允许重复**，设置**相同名称的变量，后设置的值会覆盖前设置的变量值。**
...
> 当local变量与golable变量重名,那么在当前任务结束后golable变量也将会失效
...
1. 定义流程,使用global变量控制流向

![image-20210920224945165](https://jianjiandawang.oss-cn-shanghai.aliyuncs.com/Typora/20210920224945.png)
...
>在流程图中任务节点的配置中设置 candidate-users(候选人)，多个候选人之间用逗号分开。 支持UEL表达式

![image-20210920230019672](https://jianjiandawang.oss-cn-shanghai.aliyuncs.com/Typora/20210920230019.png)	
...
- 拾取成功后变为个人任务,其他人不可再拾取(发生归还的话可以再次拾取)
...
​	![image-20210920230830044](https://jianjiandawang.oss-cn-shanghai.aliyuncs.com/Typora/20210920230830.png)
...
<img src="https://jianjiandawang.oss-cn-shanghai.aliyuncs.com/Typora/20210920231025.png" alt="image-20210920231025734" style="zoom:50%;" />`

## ParallelGateway

并行网关

![image-20210920230908304](https://jianjiandawang.oss-cn-shanghai.aliyuncs.com/Typora/20210920230908.png)	
...
<img src="https://jianjiandawang.oss-cn-shanghai.aliyuncs.com/Typora/20210920231015.png" alt="image-20210920231015622" style="zoom:50%;" />`
...
![image-20210920231357221](https://jianjiandawang.oss-cn-shanghai.aliyuncs.com/Typora/20210920231357.png)	
...
> 包含网关同样包含fork和join分支
>
> 与并行网关和排他网关不同的是,包含网关的每个分支的连线支持条件判断并可以流向多个结果为true的分支.

<img src="https://jianjiandawang.oss-cn-shanghai.aliyuncs.com/Typora/20210920231338.png" alt="image-20210920231338012" style="zoom:50%;" />	
...
![image-20210920231642451](https://jianjiandawang.oss-cn-shanghai.aliyuncs.com/Typora/20210920231642.png)	

`intermediateCatchEvent`支持的事件类型：
...
![image-20210920231711838](https://jianjiandawang.oss-cn-shanghai.aliyuncs.com/Typora/20210920231711.png)
...
Activiti7可以自动部署流程，前提是在resources目录下，创建一个新的目录`processes`，用来放置bpmn文件。

创建一个简单的Bpmn流程文件，并设置任务的用户组Candidate Groups。

> Candidate Groups为用户的组,SpringSecurity中用户包含此属性

这样填写的好处：当不确定到底由谁来负责当前任务的时候，只要是Groups内的用户都可以拾取这个任务

![image-20210920232119750](https://jianjiandawang.oss-cn-shanghai.aliyuncs.com/Typora/20210920232119.png)	
